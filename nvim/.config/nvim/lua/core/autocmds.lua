...
api.nvim_create_autocmd("FileType", {
  ...
  callback = function(event)
    ...
    vim.keymap.set("n", "q", function() api.nvim_command('close') end, { buffer = event.buf, silent = true })
  end,
})
...
api.nvim_create_autocmd(
  { "FocusGained", "BufEnter", "CursorHold", "CursorHoldI", "TermClose", "TermLeave" },
  {
    ...
    callback = function()
      -- skip cmdwin
      if fn.getcmdwintype() ~= "" then return end
      -- skip command/replace/prompt/terminal modes
      local mode = api.nvim_get_mode().mode
      if mode:match("^[cr!t]") then return end
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function()
    ...
    require('which-key').add({
      ...
      end, buffer = 0, desc = "Toggle spell checking" },
      { "<LocalLeader>p", function() vim.opt_local.spelllang = { "pt" } end, buffer = 0, desc = "Spelllang pt" },
      { "<LocalLeader>e", function() vim.opt_local.spelllang = { "en" } end, buffer = 0, desc = "Spelllang en" },
    })
  end,
})
